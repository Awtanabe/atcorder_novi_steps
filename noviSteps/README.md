...
👉 たとえば次のステップとして「全探索が間に合わない典型例」を一緒に見てみますか？（例えば **ナップサック問題**や**組み合わせ総数が 2^N になる探索**）
...
```
// ながさ 0 にすると 初期値0にはならずにメモリの再確保も行われない
make([]int, 0, 5)
```
...
- 326-like_Numbers
  - 2重ループで処理してしまっていた。桁数などが決まっていることで2重ループは不要になる


### mod 除数 ⭐️

- tokyo_festival_2019
  - 組み分けの時に使えた。インデックスをぐるぐる回る時にも使える
